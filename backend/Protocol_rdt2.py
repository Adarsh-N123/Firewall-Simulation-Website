...
class rdt_Sender(object):
	...
	def rdt_send(self,msg):
		...
		if self.state==WAITING_FOR_CALL_FROM_ABOVE:
			...
			self.packet_to_be_sent = Packet(seq_num=self.seq_num, payload=msg,ip =self.ip,port = self.port)
			self.seq_num+=1
	...
	def rdt_rcv(self,packt):
		...
		if(packt.payload=="ACK"):
			...
			print("Packet reached successfully,packet not in blacklist")
			
			self.state=WAITING_FOR_CALL_FROM_ABOVE
		elif(packt.payload=="NAK"):
			# Received a NAK. Need to resend packet.
			print("Packet Blocked,packet in blacklist")
			self.channel.udt_send(self.packet_to_be_sent)
		else:
...
class rdt_Receiver(object):
	...
	def rdt_rcv(self,packt):
		...
		if(packt.corrupted):
			...
			if (packt.ip in self.blacklist):
				response = response = Packet(seq_num=0, payload="NAK",ip=0,port = 0)
			else:

				response = Packet(seq_num=0, payload="ACK",ip=0,port=0) 
